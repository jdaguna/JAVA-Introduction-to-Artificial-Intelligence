// @Joshua Daguna, 10/20/2013 -- altered BFSearch to GreedySearch
//
// Greedy Search sorts the frontier in order from least to greatest, it depends on a heuristic function.
// this class implements Greedy Best-First search making a call to Good.Heuristic.java over the provided Heuristic.java
//
...
public class GreedySearch {
	...
	public GreedySearch(Map stateSpace, String startName, String finishName,
			int depthLimit) {
		this();
	...
	public Waypoint search(boolean useRepeatedStateChecking) {
		...
		GoodHeuristic gh = new GoodHeuristic(stateSpace.findLocation(finishName), stateSpace);
		...
		// Create a sorted frontier object ...
		SortedFrontier sFringe = new SortedFrontier(SortBy.h);
		...
		while (!(sFringe.isEmpty())) {
			...
			if (node.isFinalDestination(finishName)) {
				...
				for (Waypoint child : node.options) {
					...
					}else if (comparator.compare(child, node) == -1){
						sFringe.addSorted(child);
					}
					}
				}
