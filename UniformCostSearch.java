// @Joshua Daguna, 10/20/2013 -- altered BFSearch to Uniform Cost Search
//
// Uniform Cost Search (UCS): modifies BFS by always expanding the lowest cost node
// on the fringe using path cost function g(n) (i.e. the cost of the path from the
// initial state to the node n). Nodes maintained on queue in order of increasing path
// cost.
//
...
public class UniformCostSearch {
	...
	public UniformCostSearch(Map stateSpace, String startName,
			String finishName, int depthLimit) {
		this();
	...
	public Waypoint search(boolean useRepeatedStateChecking) {
		...
		// Create a frontier object sorted by pathcost g(n)....
		SortedFrontier sFringe = new SortedFrontier(SortBy.g);
